...
- **Modular monolith skeleton**: 13 modules exist in-tree (`src/Modules/{Account,Auth,Customer,Deletion,Disclosure,EmergencyQr,Entity,Geofence,Identity,Inventory,POS,Prescription,Sessions}/` — see `architecture/bounded-contexts/README.md` §Module → Context Mapping). The original six (`Customer,Entity,Identity,Inventory,POS,Prescription`) each contain four projects (`{Module}.{Api,Application,Domain,Infrastructure}`); the seven added by P001 (`Account,Auth,Sessions,Deletion,Disclosure,Geofence,EmergencyQr` — Identity & Access / Personal Health contexts) are out of Phase 0 scope; each Application layer already has `DependencyInjection.cs` registering MediatR + FluentValidation via `RegisterServicesFromAssembly(AssemblyReference.Assembly)`; each Api layer has `ModuleRegistration.cs` exposing `Add{Module}Module`; each Infrastructure layer has `DependencyInjection.cs` exposing `Add{Module}Infrastructure(IConfiguration)` that calls `services.AddDbContext<{Module}DbContext>(o => o.ConfigureDatabase(dbOptions))`. Domain + Application directories are empty except for `AssemblyReference.cs`.
...
The delta required by the spec is therefore: (1) fill the empty Entity + Identity Domain / Application layers with the Workspace, EntityRoot, EntityType, Branch, AdminUserMirror aggregates/entities + the Identity-owned `LockoutRecord`, and the CQRS handlers + validators + controllers (commands are intent-revealing and mutate only via aggregate methods); (2) add cross-cutting `Balsm.Infrastructure.{Lifecycle, Backup, Audit, Operations, Localization}` namespaces + an `AuditSaveChangesInterceptor` + a `MigrationGateMiddleware` + WAL PRAGMA wiring in `DatabaseServiceExtensions`; (3) extend `AdminAuthService` from PBKDF2 to Argon2id (lazy migration on next login) and add a one-time recovery code surface (recovery-code material is credential-file state managed by a Supervisor `RecoveryCodeService` — NOT a domain aggregate; Identity & Access is a Generic subdomain and stays lightweight per Constitution Principle IV); (4) add system-tray UI cards / pages (Entities, Backups, Audit, Recovery) to `admin-ui/` plus i18n bundles for English + Arabic with RTL; (5) wire two new hosted services (`BackupScheduler`, `AuditRetentionJob`) into the existing `AddSharedInfrastructure` registration.
...
|-----------------|----------------|------|--------|--------------------|--------------|
| Entity Management | Provider | Balsm-API-DotNet | src/Modules/Entity | Domain (Workspace, EntityRoot, EntityType, Branch aggregates + WorkspaceStatus + domain events) + Application (intent-revealing CQRS) + Api (controllers) | Existing skeleton, filled in-place |
| Identity & Access | Cross-plane | Balsm-API-DotNet | src/Modules/Identity | Domain (AdminUserMirror, LockoutRecord) + Application + Infrastructure (IdentityDbContext) | Existing skeleton, filled in-place |
| Identity & Access | Cross-plane | Balsm-API-DotNet | src/Balsm.Supervisor | Auth (AdminAuthService PBKDF2→Argon2id, FileCredentialStore, AdminSessionService, RecoveryCodeService — file-store credential material, no domain aggregate) + Middleware | Existing, extended |
| — (shared kernel / infra, sanctioned non-context value `infrastructure`) | — | Balsm-API-DotNet | src/Balsm.Infrastructure | Lifecycle, Backup, Audit, Operations (EF schema `ops` — NOT "Platform", reserved for the canonical Platform plane), Localization namespaces; AuditSaveChangesInterceptor; MigrationGateMiddleware; WAL wiring | New namespaces |
...
- Reuse cookie auth (`balsm_admin_session`) for `/api/v1/admin/*`. Do **not** introduce JWT for the local admin panel at Phase 0 — JWT remains for Federation / Network / Public clients outside the foundation surface. The cookie MUST be `HttpOnly; Secure; SameSite=Strict; Path=/`, and because it is a cookie-authenticated SPA, `AdminAuthMiddleware` MUST require a custom `X-Balsm-Csrf: 1` header on every state-changing `/api/v1/admin/*` request (custom-header CSRF defense; T038b).
- Plain HTTP `:5050` MUST serve only `307 → HTTPS` redirects + `/api/v1/health`; credentials/API traffic never transit cleartext (HSTS on `:5051`). The mDNS `http_port` field is a discovery hint, not an API fallback.
- Secrets/logging: Serilog MUST NEVER log request/response bodies of `/api/v1/admin/auth/*` (passwords, one-time recovery codes). Backups are AES-256-GCM-encrypted at rest (or FDE is a documented prerequisite); the backup + audit-archive directory is `0700`/service-user-only; the credential file is `0600`/SYSTEM-only.
- IP-derived controls (rate limit, lockout, audit `SourceIp`) MUST use the real client IP via `ForwardedHeadersMiddleware` trusting only the tunnel hop in Public mode — never the loopback proxy address.
...
| IV. Modular Monolith with DDD | PASS | New work strictly inside existing module boundaries (`Modules/Entity`, `Modules/Identity`) plus cross-cutting infra namespaces; no cross-module DB access. Contexts mapped in §Module & Bounded Context Mapping — Entity Management (primary, Provider plane) + Identity & Access (Cross-plane) per `architecture/bounded-contexts/` canvases; aggregates small (Workspace, EntityRoot, Branch, AdminUserMirror), cross-aggregate refs by ID, invariants + domain events on the model, commands intent-revealing (`RenameWorkspaceCommand`, `ChangeWorkspaceSlugCommand`, `Deactivate…`/`Reactivate…`). Identity & Access is Generic — kept lightweight: recovery-code = credential-file material via `RecoveryCodeService`, no aggregate; `LockoutRecord` state owned by the Identity module. Ops state is namespaced `Operations`/`ops`, avoiding collision with the canonical Platform plane |
...
│   │   ├── Operations/                                                     # (new) local-server ops state — EF schema `ops` (NOT "Platform": reserved for the canonical Platform plane)
│   │   │   ├── ServerConfigEntry.cs                                        # (new) key→value config store (data-model A.10)
│   │   │   ├── OperationsDbContext.cs                                      # (new) DbSets: ServerConfigEntry, BackupFile, AuditLog, AuditArchive, MigrationStateRecord
│   │   │   ├── OperationsSeedData.cs                                       # (new) 13 seeded ServerConfig keys
│   │   │   └── Configurations/                                             # (new) EF configurations for the ops entities
...
│       ├── Identity/
│       │   ├── Balsm.Identity.Domain/                                      # (empty scaffold) — add AdminUserMirror (no password material) + LockoutRecord (lockout is Identity & Access language — data-model A.11)
│       │   ├── Balsm.Identity.Application/                                  # (empty scaffold) — add EnforceSingleAdminPolicy + UpdateAdminLocaleCommand
│       │   ├── Balsm.Identity.Infrastructure/                              # (existing) IdentityDbContext — extend with AdminUserMirror (filtered unique index) + LockoutRecord entities
...
│       ├── Customer / Inventory / POS / Prescription                        # (existing — untouched at Phase 0; map to Customer Relations / Inventory / Point of Sale / Prescriptions contexts)
│       ├── Account / Auth / Sessions / Deletion / Disclosure / Geofence     # (existing — P001 Identity & Access modules; untouched at Phase 0)
│       ├── EmergencyQr                                                      # (existing — P001 Personal Health module; untouched at Phase 0)
