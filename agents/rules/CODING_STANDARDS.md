...
- **list building: `.map()` over collection-`for`** — when transforming a collection into list elements (widget children, DTO lists), use `.map(...)` (spread `...xs.map(...)` inside literals, or `.map(...).toList()`), `.where(...).map(...)` for filtered transforms, and `.indexed.map((e) => ...)` when the index is needed (`e.$1` index, `e.$2` item). Collection-`for` stays ONLY where `.map` cannot express the loop without changing semantics or readability: sequential `await` bodies, `sync*`/`async*` generators, numeric ranges with no backing list, early `break`/`continue`/`return`, multi-collection accumulation, buffer mutation, map/set literals (`{for (...) k: v}`), and record-pattern destructuring that named bindings make clearer. (2026-07-24 directive; repo-wide sweep applied in balsm_app_flutter.)
- **domain state holds value objects, not raw strings** — model/state fields use the typed value object (`LanguageCode lang`, `CountryCode country`), derive booleans from it (`lang.isRtl`, `country == kHomeCountry`), and parse untrusted/stored input at the boundary via the named factories: `fromCode` (throws on malformed) / `tryFromCode` or `tryParseUi` (null → caller falls back to a domain constant). Unnamed value-object factories are not used.
